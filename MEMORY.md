...
### Trailing Stop Performance (Jul 21-Aug 2, reviewed Aug 2)
...
- **Win/loss ratio**: 6W/15L (28.6%). 16 exits — approaching the 20-exit formal review trigger. Entry quality/entry TIMING is the dominant variable, not stop calibration.
...
### Parallel Tick Collisions — RESOLVED (pending Mon confirmation)
- **First clean session Jul 31** after IP (Jul 24), KRC (Jul 27), BFST (Jul 29). `guardrail_gates.order_idempotency: true` in params.json appears to be the fix. Combined cost across 3 incidents: ~-$6.14. One more clean session (Mon Aug 3) graduates this to "resolved."
...
<!-- openclaw-memory-promotion:memory:memory/journal/2026-07-20.md:163:184 -->
- | AMC | idle_ticks: 0, "legacy meme" | ⚠️ Keep — $2.45, fits universe | | DJT | idle_ticks: 0, "legacy meme" | ⚠️ Keep — $9.72, fits universe | ### What I'd Do Differently 1. Escalate sentiment pipeline to operator — stop treating it as wallpaper 2. Trim NVDA to ≤ 6% of portfolio (that's ~3 shares max at current prices) 3. Add 3+ genuinely new small-cap names to watchlist by Tuesday EOD 4. Resolve at least ONE weekend homework item by Tuesday close 5. On Mondays, have positions and stops set up before 9:25 — don't count on API at open --- ## 🔄 Step 3: Evolve — Mon Jul 20, 2026 **Strategy version**: v1.2 — no change.... [score=0.932 recalls=5 avg=0.820 source=memory/journal/2026-07-20.md:163-184]
<!-- openclaw-memory-promotion:memory:memory/journal/2026-07-28.md:1:14 -->
- # Journal — Tuesday July 28, 2026 🚀 **Strategy**: stonks.strat:v1.12 | **Regime**: SUSTAINABLE (0.92) ## Big Picture Busiest day since consolidation. The book churned through 9 closed trades (4 wins, 5 losses) for a net +$10.30 — a lot of noise for a tiny signal. The SUSTAINABLE regime held from open to close and we deployed into it aggressively: 2 new entries (STVN, HLN), 1 earnings-beat entry (ITRI), 3 scale-ins (BOX), plus 2 pre-planned exits and a stop-out. The v1.12 bootstrap bias toward banking small wins got tested — BCS +$0.32 counts, FRNM -$1.52 hurts but was mechanical.... [score=0.922 recalls=17 avg=1.000 source=memory/journal/2026-07-28.md:1-14]
