...
**Size discipline (2026-08-12, now that this file is auto-tier self-commit, see `skills/decision-tree.md`)**: this file is read fresh every tick, so its size is a real per-tick cost, not just tidiness — and with self-commit there's no human reviewer left to notice it creeping up. Soft cap **35,000 chars** (`wc -c decision_heuristics.md`; ~21KB as of 2026-08-12, real headroom, not an immediate constraint). Before self-committing a new/promoted `Active` node during nightly Evolve, check current size — at or over the cap, this cycle's addition must be paired with at least one `skills/rule-mechanization-audit.md` 🗑️ removal candidate (superseded/dropped-version/never-mattered) in the same commit, not deferred to "later." Trimming an existing node's prose for concision (without changing its Trigger/Recommended action/Evidence) is always fair game and doesn't need a removal to justify it.
...
**Sizing guidance**: exempt from `risk.conviction_play.position_size_pct` on the upside — an index-anchor position sitting above that cap organically is not a breach (`check_stops()` does not trim it, `INDEX_ANCHOR_TICKERS` in `executor.py`). **Current policy (2026-08-10, Raf's direction): do not actively add to an index-anchor position beyond its current size.** When a new opportunity needs cash, use `index_anchor_reallocation_v1` (sell it down) instead of deploying fresh capital alongside it. Earmark 2 of the 5 conviction slots for index-anchors, never more than 3 — leave at least 2 free for individual-stock conviction picks. This exemption only covers a position already held growing past the cap organically — the *initial* BUY still has to clear `gate_position_size` at order time, so a 1-share-minimum ETF whose price alone exceeds `max_position_pct`% of current equity isn't orderable yet at all, exemption or not (e.g. at ~$10.4K equity, 1 SPY share is ~7.4%, over the 6% cap — SPY isn't a live option until equity grows past ~$12.9K or SPY drops under ~$625; IWM/QQQ/DIA fit fine at that size in the meantime).
...
### NODE small_cap_earnings_caveat_v1

**Status**: active
**Type**: risk/refinement
**Origin**: evolved from live tracking across 3 sessions (Aug 11-13 nightly learning → nightly-maintenance, 2026-08-13). Not a gate, an evidence-gated caveat that shifts the sizing/conviction posture for one specific entry pattern.
**Trigger**: a candidate's primary entry rationale is a small-cap earnings beat (sub-$2B market cap, Q2/Q4 earnings headline), AND the sole stated edge is the headline beat itself — no independent confirming signal (volume > 2x normal, MACD acceleration/divergence, specific sector tailwind, or a second catalyst beyond the earnings print).
**Recommended action**: HOLD (do not enter on the beat alone). A small-cap earnings beat without at least one additional confirming signal is not sufficient entry rationale — the live pattern (4 instances, 0 counter-examples) shows these beats reliably fail to produce follow-through buying. The candidate may still be entered if an independent confirming signal exists — state it explicitly in `--rationale`, tag `[tree:small_cap_earnings_caveat_v1 cleared: <confirming signal>]` or `[tree:small_cap_earnings_caveat_v1 match, HOLD]` depending on the outcome.
**Conviction tier**: probe (forced) if overridden without a stated confirming signal — this is a deliberate downward sizing cap: an earnings-beat entry that doesn't clear this node's bar gets probe-tier sizing at most, not standard.
**Sizing guidance**: if cleared (confirming signal present), proceed at normal sizing for the underlying setup (standard/conservative per `catalyst_led_entry_v1`). If overridden without a stated confirming signal, cap at `probe` tier regardless.
**Evidence**: 4 live instances across 3 days, zero counter-examples where a sub-$2B company earnings beat produced sustained follow-through buying without an additional confirming signal. HPK (Aug 11, beat → -5.56%, entered against tree rejection — the beat alone wasn't enough), UPB (Aug 12, beat → immediately sold off -4.7%, not entered — correct skip on beat alone, but the framework's highest-confidence overnight call was still wrong), MBBC (Aug 5, beat → post-earnings drift to -0.28%, entered — the beat didn't produce follow-through), INAB (Aug 13, beat → sold off to $0.88 intraday low, correctly skipped — 4th instance confirming the pattern). Contrast: GEL (Aug 11, energy mid-cap with sector tailwind from elevated oil + volume 3.7x confirming) cleared this node's bar with two independent confirming signals — the catalyst wasn't just the earnings beat, it was the sector + volume convergence. AVAH (Aug 13, Q2 beat +94% profit + raised full-year guidance + P/E 7.78 + price -4.7% below MA20) cleared with multiple confirming signals — the specific guidance raise and the valuation discount were independent edges beyond the headline beat.
**Confidence (tracked)**: insufficient_data (n=0 tagged as of ship — the 4 historical occurrences predate this node). Tag `[tree:small_cap_earnings_caveat_v1 match|cleared]` on future earnings-catalyst evaluations so `tree_scorecard.py` can track the hit rate.
**Override note**: may override the HOLD with a stated, specific confirming signal — volume > 2x, MACD acceleration, sector tailwind, separate non-earnings catalyst, or (for mid/large-caps >$2B or stocks with an established sector trend like GEL in energy) a stated reason why this beat is different from the small-cap sell-off pattern. An override that cites only the earnings beat itself without any additional confirming signal is implicitly a `deployment_pressure_override_v0` match (the HOLD leaf there takes priority).
