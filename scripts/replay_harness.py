...
Vendored 2026-08-10 from paper-trading-rebuild/src/replay.py (self-contained
-- stdlib + numpy only, no other cross-repo deps) so replay_check.py and the
tests that import it don't depend on a sibling repo only present on this one
machine's filesystem, which broke test collection entirely in CI.

Usage:
    from replay_harness import Tick, TraderDecision, replay_trader
"""
