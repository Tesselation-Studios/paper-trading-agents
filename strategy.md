...
Trade small and wide, not concentrated — volume of decisions and honest feedback over a hero trade. Qualitative judgment that keeps proving out gets promoted into `params.json`/code. Promotion requires the backtest to actually hold up — v1.3 reverted two rounds of additions (v1.1, v1.2) after a corrected backtest showed the added complexity underperforming the simpler baseline on raw return. v1.4 re-promoted v1.1's exit/entry rules after a stronger re-test (split-window Sharpe, not raw aggregate return) showed they actually do hold up. v1.2's rules remain reverted — no comparable re-test evidence exists for those. See Version History.
...
- **Entry signal**: rising RSI in the 45-65 band + volume + a real catalyst, skipped when 20-day realized volatility is elevated (CHOPPY/high-vol regime — re-promoted from v1.1, v1.4). No triple-confirmation, no sector-ETF veto, no VIX-tiered sizing — those stay reverted, no evidence ever supported them.
- **Exit**: MACD histogram flip (positive→negative) triggers an immediate exit — re-promoted from v1.1, v1.4. Otherwise fixed stop-loss/profit-target from `params.json`, full exit (not partial trim), or thesis breaks. Never chase a peaked pump. Hard stop and trailing stop are mechanically enforced in `executor.py`'s guardrail gates regardless of strategy version — always on, not something this file's prose controls.
...
- **Re-promoted in v1.4** (supersedes the v1.3 revert note below): "MACDh flip = reliable near-term exit" and "regime-gating entries works" were reverted in v1.3 after a corrected 3-night backtest showed v1.0 beating v1.1 on raw return. Re-tested 2026-07-23 with a stronger bar — split-window Sharpe (must be positive in BOTH halves of the window independently, not just the full-window aggregate) — across three separate checks: the live 11-ticker universe, a 200-ticker wide scan, and a chronological split of the live-universe window. v1.1 was the only variant Sharpe-positive in both halves every time; v1.0 (the reverted-to baseline) flipped sign between halves (+1.04 → -1.31) — its earlier good half was propping up the aggregate number. A 25-combo stop-loss/profit-target sweep found no parameter-only fix (0/25 robust) — confirms the problem was v1.0's rule set, not calibration. Raw-return-on-one-aggregate-window was the wrong bar; split-window Sharpe is the bar going forward. Old note, kept for history: "a corrected 3-night backtest (fixed a MACD column-index bug that had been silently testing the wrong signal) showed v1.0's simple baseline outperforming v1.1 on raw return in 2 of 3 runs."
...
- `v1.4` (2026-07-23) — Re-promoted v1.1's MACDh-flip exit + regime-gated entries (skip entries in elevated 20-day volatility), reversing the v1.3 revert. Evidence: split-window backtest (Sharpe positive in both halves independently, not just full-window aggregate) confirms v1.1 across three separate checks — live 11-ticker universe (full-window Sharpe 0.916, +1.65/+0.82 both halves vs v1.0's +1.04/-1.31 sign-flip between halves), a 200-ticker wide scan, and this chronological split. A 25-combo stop-loss/profit-target sweep found none robust (0/25 Sharpe-positive-in-both-halves) — confirms the problem was v1.0's rule set, not parameter calibration. v1.2's triple-confirmation/quality-gate/VIX-tiering stay reverted — no new evidence for those. All v1.3/v1.3.1 operational hardening (GTC audit, account audit, deployment checklist) unchanged. Proposed by the new stonks-evolution-batch cron's first real run, independently confirming the same evidence found earlier the same day — applied here after explicit sign-off, not auto-applied, per the still-being-earned trust in that pipeline.
