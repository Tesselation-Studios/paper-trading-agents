# Stonks — Strategy stonks.strat:v1.13
...
- **Entry signal**: rising RSI in the 40-70 band + volume > 1.0x avg (or strong price momentum if volume is flat). No catalyst requirement — RSI + volume/momentum is sufficient. **Regime still matters but no longer gates entries** — CHOPPY regime: size down to 1-3 shares probe size, use the dynamic `conviction_floor_min` (0.25) instead of the standard floor. Clear/SUSTAINABLE regime: normal sizing at `conviction_floor` (0.35). Weigh `get_fundamentals`/`get_insiders` when available but skip any that error (`get_flow` disabled — unusual_whales RSS blog, not actual options flow). Strong confluence adds confidence but empty data doesn't block a trade that clears the base gates. No triple-confirmation, no sector-ETF veto, no VIX-tiered sizing.
  - 2026-07-29: RSI band widened 45-65→40-70, catalyst requirement dropped, CHOPPY gate removed (was: no entry in choppy), conviction floor lowered per overnight optimization (99%+ cash idle across all variants despite signal quality 0.65-0.76).
  - 2026-07-30: conviction floor lowered further 0.40→0.35, min 0.30→0.25 per overnight backtest — middle ground between v1.0's 60.6% win rate and v1.7's 39.1%.
- **Exit**: Hard stop-loss (`risk.stop_loss_pct`, currently -10%) is a non-negotiable full exit. Volatility-scaled trailing stop ratchets up from peak price (TRAIL_K=40, clamped 4-12% based on 20d realized vol — wider for noisier tickers, tighter for stable ones). Profit-target (`risk.profit_target_pct`) is a **guide, not an automatic trigger** — near it, use judgment. Also exit on thesis breaks. MACDh-flip is NO LONGER a mandatory exit (v1.13 — removed 2026-07-30: backtest evidence shows v1.0 without it beats all versions with it). Never chase a peaked pump. All stops are mechanically enforced in `executor.py`'s guardrail gates.
