# Stonks — Strategy stonks.strat:v1.10
...
- **Degraded-data entries (v1.10)**: data-bus down (empty quotes/momentum null) is not a reason to sit out entirely — RSI just can't be confirmed, so it stops counting toward the signal. Substitute stronger confluence on what's still live: real price action (holding a level, breaking out on the last known prints) + a real catalyst + at least one of `get_flow`/`get_insiders`/`get_fundamentals` in agreement (these don't depend on the data bus). Probe size only (1-3 shares) while degraded, never normal size, regardless of regime reading — this is a lower-confidence entry by construction, not a fully-confirmed one. Note the degraded-data condition in the `record_decision.py` rationale so these entries stay reviewable/separable from normal ones.
