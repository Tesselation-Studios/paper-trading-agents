...
6. **Scan positions near triggers** → run `python3 scripts/executor.py --account stonks --action check-stops` — mechanically checks every open position against the hard stop (`risk.stop_loss_pct`) and trailing stop (`risk.trailing_stop_pct`, ratchets up from peak price since entry). Both distances now widen the longer a position is held (up to `stop_loss_pct_max`/`trailing_stop_pct_max`), compressing back toward the base distance near the competition deadline — the breach `reason` string always states the *effective* distance and days held for that check, not the flat params.json number, so read it rather than assuming. Also checks the oversized-position cap (`risk.max_position_pct` — a position that's grown over cap via price appreciation, not a new buy). Any ticker returned in `breaches` **must** be sold this tick via step 9, no re-litigating — for `stop_type: "oversized"`, sell exactly the `shares_to_sell` count given (a trim, not a full exit) unless another breach on the same ticker calls for a full exit instead. Also check profit targets, thesis breaks — read `positions/*.md` only for names near a trigger.
...
9. **Execute** → via executor (`skills/tool-invocation.md`) if trade — pass `--price`/`--conviction`/`--sector` so the executor's built-in guardrail check (position size, max positions, sector concentration, market hours, conviction floor, bankroll ceiling) can evaluate it; a rejected order exits non-zero with the blocking gate's reason — do not retry the same trade, note it in active.md and move on. On any BUY/SELL (not routine HOLD), log the decision via `record_decision.py decision` (same skill) with the same `--conviction`/`--features` from step 8's reconcile call — see `skills/self-improving-agent.md` for exactly how to score each signal and what the echoed-back `reconciled` result means. If the SELL closes a position, also log the outcome (pnl/return_pct from entry vs. exit).

   **`positions/*.md` is thesis storage, not a price mirror** (2026-07-27): only rewrite a ticker's file when the *thesis itself* changes — new entry, exit, a real reasoning update (stop-loss note, conviction shift, catalyst development). Current price/market value/unrealized P&L do **not** belong in this file at all — Alpaca is already the live source of truth for those (step 4 fetches them fresh every tick; `check_stops`/`status` never read this file for pricing). Rewriting the file every tick just to refresh a number that's already available live was generating a git commit+push every ~5 minutes for zero new information, hammering the shared remote pointlessly and colliding with other work on the same branch. If you're about to write a position file and the only thing that changed since last time is price/timestamp, don't.
