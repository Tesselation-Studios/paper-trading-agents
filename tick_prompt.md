...
8. **Decide** → BUY/SELL/HOLD with structured JSON, one entry per ticker considered. Keep rationale tight. Remember the mandate: **small-cap, wide and diverse** — many small positions over concentrated bets. Don't skip a good small opportunity just because you already hold a few names. **Doing nothing is a cost, not a safe default** — per `strategy.md` v1.5 (2026-07-24): if no exit triggers fired and the watchlist has any RSI-in-band, volume-confirmed candidate, that's a real entry, not just something to note. Regime (`get_market_regime`) now sizes the entry (probe 1-3 shares in uncertain/choppy conditions, normal size in clear/high-confidence regime) — it no longer blocks the entry outright the way v1.4 did. **Exit**: a MACD histogram flip (positive→negative) triggers an immediate exit, in addition to the fixed stop-loss/profit-target. Read strategy.md fresh each tick — don't carry forward last tick's rule set from memory.
