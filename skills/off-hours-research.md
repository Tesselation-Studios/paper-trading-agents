...
| Step | Command | Output |
|---|---|---|
| 1. News + sentiment | `python3 scripts/news_collector.py <tickers>` | Fresh RSS → local `news_cache` table (`state/trader.db`, additive). Sentiment over last 24h. Note real catalysts, not noise. |
...
| 3. Signal scorecard | `python3 scripts/signal_scorecard.py` | Real empirical hit rate per signal from `state/trader.db`'s `training_examples` table. Signals under 10 labeled examples show `insufficient_data`, not a fake precise number — that's expected while volume is low, not a bug. |
