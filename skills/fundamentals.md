...
**Known gap**: may return `"error": "no data available"` for every symbol, including mega-caps — the underlying `skill_combo_fetch` module can be missing from the engine repo's venv, and the yfinance fallback can be blocked from this homelab's network. Treat empty fundamentals as expected in that state — skip on error like every other data-bus call here, never block a trade on it.
