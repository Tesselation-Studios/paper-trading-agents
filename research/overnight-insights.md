...
# Overnight Optimization — 2026-08-10/11 Insights

**Run date**: 2026-08-10 overnight → 2026-08-11
**Configs tested**: 9 runs (2 × 50 variants, 7 × 15 variants)
**Total signals**: thousands across all runs

---

## The Big Finding: Loose Conviction on Stonks, 0% False Positives

Run 5 (stonks-loose-conviction) is the headline: **4.45% catch rate with 0.00% false positives** on 10 stonks over 10 days. That's the highest catch rate of any run, and it's doing it with zero false triggers. The config is simple: conviction threshold 0.5 (loose), 7-period RSI (short, momentum-sensitive), RSI range 55-75.

Compare this to previous nights where catch rates hovered around 0.4-1.5%. The difference is the conviction threshold — dropping from 0.7-0.8 to 0.5 opens up the signal pool dramatically without introducing noise. That's not what I expected. I assumed looser conviction would mean more false positives. The data says otherwise: the false positives are coming from somewhere else (mean reversion params, ultra-short windows), not from conviction gating.

**Actionable**: The 7-period RSI on volatile names is worth trying in live trading. The current strategy uses standard 14-period RSI. A 7-period RSI on small-cap momentum names would catch the early move that 14-period misses. This isn't a strategy change — it's a signal sensitivity tuning. Same thesis, faster trigger.

## Tech MA-Sensitive: Best Returns, Quiet Signals

Run 4 (tech-ma-sensitive) delivered **6.71% return** on 7 tech names over 10 days — the best return of any run outside the core config. The config uses tight MA proximity (2% threshold) with a 10-bar MA. It's catching the pullback-to-MA-then-resume pattern that tech stocks do in uptrends.

This matters because Technology is our best sector (75% win rate, 6W/2L) and we have zero pipeline coverage there. The 10-bar MA with 2% proximity is a concrete signal filter we can use when the discovery daemon actually finds tech names. Combined with the 7-period RSI from Run 5, we'd be looking for: tech stock within 2% of its 10-day MA, RSI 55-75 on a 7-period lookback, with volume at least 1.5x normal. That's a tight, specific setup.

## Core Default: The Workhorse

Run 1 (core-default) with MACD fast12/slow26, RSI 50-70, volume 2.0x, MA prox enabled gave the best blended score (0.2544) with 10.98% return. This is the canonical setup and it keeps working. Nothing new here, just confirmation that the default params aren't broken. The blended score is still being inflated by the scoring function's stability bias (see Aug 9 insight), but 10.98% return on 20 days of data is real.

## What Doesn't Work (Again)

- **Mean reversion on volatile stocks** (Run 8): 23.08% false positive rate. RSI 25/80 with 8% MA distance is a trap on small caps — you catch the falling knife, not the bounce. This has been consistently bad across multiple overnight runs.
- **3-day ultra-short windows** (Run 9): 0.28% catch, 0.55% return. Too noisy. The signal-to-noise ratio in a 3-day window on 30 tickers is essentially random.
- **Finance/catalyst-heavy** (Run 7): 0.00% catch. The params are too conservative — catalyst min 0.3 with high conviction bar filters everything out.
- **price_above_ma on small caps** (consistent across multiple runs): the Aug 9 finding holds — dropping this filter improves catch rate without introducing false positives on sub-$500M names.

## Stan's Read: The CHOPPY Reality Check

Here's what these results look like through the lens of a Monday where we finally deployed the index-anchor after 14 sessions of 93% cash:

The overnight optimizer is finding the same thing I'm finding in live trading — there ARE real signals on volatile names (4.45% catch, 0% FP), but they're sparse and require looser conviction gates to access. The core default works but it's conservative. The tech MA-sensitive setup works but needs actual tech names in the pipeline (we have zero).

The real lesson from tonight's runs isn't a parameter tweak — it's that **the signal discovery machinery is producing valid signals that the live trading strategy isn't configured to catch.** The 7-period RSI, the loose 0.5 conviction threshold, the tight MA proximity on tech — these are all tuning knobs that the optimizer says would improve catch rates without adding noise. They're worth testing in live trading as experimental variants, not wholesale strategy changes.

**What I'd change for the morning session:**
1. Watch for volatile/small-cap names where a 7-period RSI is materially different from 14-period — the 7-period picks up the early move that 14-period smooths out
2. When (not if) tech names finally appear in the unblinded pipeline, screen them against the MA-proximity setup: within 2% of 10-day MA, RSI 55-75, volume 1.5x+
3. Don't fight the core default — MACD fast12/slow26 + RSI 50-70 + volume 2.0x still works, just catches fewer signals in CHOPPY
4. The 0% FP on loose conviction is the single most interesting number — it suggests I can widen the entry funnel on volatile names without adding garbage

_Generated by Stan Hoolihan, 2026-08-10 overnight cycle (9 runs). Reviewed before the Aug 11 session._

---

# Overnight Optimization — 2026-08-12/13 Insights

**Run date**: 2026-08-13 01:40–02:14 ET
**Configs tested**: 12 runs, 360 variants, 1h-sampled bars, split-window validation
**Previous run**: 2026-08-10/11 (9 runs)

---

## The universe reversal is the headline

Two nights ago I wrote the core small-cap universe "returns 2.4x+ more than broad/stonks" and that the stonks basket was academic "if we're not trading stonks names." Tonight that flipped:

| Universe | Best score | Return | Win | Trades |
|---|---|---|---|---|
| **Stonks** (NVDA/TSLA/COIN/PLTR/MSTR/GME/RIOT/MARA/HOOD/DJT) | **0.2753** | **+14.2%** | **75%** | 8 |
| Core small-cap | 0.2356 | +6.0% | 47% | 17 |
| Large-cap | 0.2021 | — | — | — |
| All-tickers | 0.1978 | — | — | — |

The high-beta momentum basket is now the clear winner, and the ONLY configs clearing 50% win rate were stonks (75%, and 75% on a second stonks variant). This is not a margin call — it's a reversal.

**But I don't trust it yet, and the run itself is why**: the split-window validation explicitly reports our configs "don't generalize well between halves." So last night's "core wins" and tonight's "stonks wins" may both be regime artifacts, not stable edges. The one durable truth across all three nights is that these two universes *behave differently* — core small-caps mean-revert, stonks momentum names trend — and the same gates can't serve both. That's the finding worth keeping, not the specific winner of any given night.

Zooming out to the 3-night trend though: Aug 10/11 already flagged "7-period RSI on volatile names" + "loose conviction 0% FP" on stonks, and tonight lands the same direction. The momentum/high-beta basket keeps out-scoring small-cap mean-reversion. This is the same thread my memory already carries — "small-cap consistently underperforms large-cap" (Jul 31), "core returns 2.4x more" (Aug 9/10, now contradicted), and tonight. Three nights, two pointing momentum. One pointing core. Not conclusive, but the universe question is now a genuine open thread, not a passing note.

## Selective beats aggressive — but 8 trades is noise

| Config | Trades | Win | Return |
|---|---|---|---|
| stonks-relaxed | 8 | 75% | +14.2% |
| stonks-aggressive | 28 | 46% | +10.0% |
| all-momentum/all-tight | 132 | 40% | +24.4% (volume play, not precision) |

"Selective beats aggressive" is the Nth time this has surfaced. Lower trade counts → higher precision. That much is consistent. But 8 trades at 75% is statistically nothing — a coin flipped 8 times lands 6 heads all the time. I'm not going to over-rotate my "small and wide, rack up reps" mandate off an n=8 sample.

The more honest read of the 132-trade/24.4% line is that **the raw return ceiling is in volume, not precision** — but that volume comes at 40% win and 0.33 FP, which is exactly the "quantity not quality" trap the optimizer itself warns about. I'll take the precision signal as directional (be more selective on entry quality) and leave the volume play alone.

## Cash-idle is immovable (Nth consecutive run)

96–99% cash across every config, every run, all universes. This is structural now, not a parameter problem. Two causes I've already diagnosed and don't need to re-litigate: (1) the scoring function rewards stability/low-variance over raw P&L, so the optimizer steers toward "sit in cash, occasionally make a small smooth profit," and (2) tight gate stacks compound into near-total idleness. The fix was never more gate tuning — it's deployment pressure in live trading, which is a process/psychology lever, not a params knob. Tonight changes nothing there.

## FP rate is the real bottleneck — conviction doesn't fix it

Best false-positive rate is 0.11 (stonks-relaxed variants); the norm is 0.33–0.69. That's the honest scoreboard: even the "good" configs are wrong a third to two-thirds of the time. And the run gives me the cleanest confirmation yet of a lesson already in my memory: **high conviction ≠ accuracy** — 0.75 conviction still gets 0.17 FP. Raising the conviction bar alone does not buy precision. Same for wide RSI bands (28–78 = worse, more noise) and short-momentum (3-bar) + tight MA (8-period), which was the worst config across every metric.

The consistent conclusion across all these nights: **the raw signal stream is the bottleneck, and no gate tuning — tight or loose, conviction or RSI — changes that.** The only lever that's ever moved catch/FP is a quality pre-filter at the discovery level. That's been true since Aug 9 and it's still true tonight.

## What I'd actually do (cautious, not wholesale)

1. **Don't touch strategy/params off this run.** Split-window non-generalization is the honest headline — the winner is regime-dependent, and I'm not rebuilding the playbook on 1h-sampled, small-n data that contradicts itself night to night.
2. **Open the universe question as a tracked thread, not an action.** Momentum/high-beta vs small-cap mean-reversion now has 2-of-3 nights pointing momentum. That's worth a deliberate, separate investigation — not a same-day flip. I'll file it in `tasks/pending.md` for a focused look rather than let it quietly drift.
3. **Keep the one durable lever in view**: discovery-level quality pre-filter, not gate tuning. Every run reconfirms it; nothing has ever contradicted it.
4. **Treat "selective > aggressive" as an entry-discipline nudge only** — weigh the gestalt harder before pulling the trigger, don't mechanically cut trade count.

_Generated by Stan Hoolihan, 2026-08-13 overnight cycle (12 runs, 360 variants). The run's one genuinely new fact — the universe reversal — is flagged as an open thread, not acted on. Nothing here clears the split-window bar for a strategy change._
